...
const LineCharts = ({ chartInfo }) => {
  ...
  // Region config: English keys and localized display names
  const regions = useMemo(
    () =>
      language === "ge"
        ? [
            { key: "GEO", name: "საქართველო" },
            { key: "TBILISI", name: "თბილისი" },
            { key: "GURIA", name: "გურია" },
            { key: "RACHA_LECHKHUMI", name: "რაჭა-ლეჩხუმი" },
          ]
        : [
            { key: "GEO", name: "Georgia" },
            { key: "TBILISI", name: "Tbilisi" },
            { key: "GURIA", name: "Guria" },
            { key: "RACHA_LECHKHUMI", name: "Racha-Lechkhumi" },
          ],
    [language]
  );
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        ...
        // Fixed variable keyName for Annual average temperature
        const variableKeyName = "Annual average temperature (°C)";
        ...
        const processedData = yearData
          .map(({ year }) => {
            ...
            regions.forEach((region) => {
              // Construct full key: "REGION - Variable"
              const fullKey = `${region.key} - ${variableKeyName}`;
