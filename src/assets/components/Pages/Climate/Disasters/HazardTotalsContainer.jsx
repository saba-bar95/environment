...
function transformDataForTotalsChart(apiData, apiMetadata) {
    ...
    const hazardMeta = apiMetadata.data.metadata.variables.find(v => v.code === "Hydrometeorological hazard");
    ...
    filteredYearsData.forEach(yearData => {
        ...
        for (let hazardIndex = 0; hazardIndex < hazardMeta.values.length; hazardIndex++) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const transformedData = transformDataForTotalsChart(apiData, apiMetadata);
