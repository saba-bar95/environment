...
const PieCharts = ({ chartInfo }) => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            try {
                ...
                const yearDataFromMeta = metaDataResult?.data?.metadata?.variables[1]?.valueTexts || [];
                setYears(yearDataFromMeta);

                if (yearDataFromMeta.length > 0) {
                    const latestYear = Math.max(...yearDataFromMeta.map(Number)).toString();
                    setYear(latestYear);
                }
    ...
    // Empty state
    if (pieData.length === 0 && !isLoading) { // Added !isLoading to prevent showing "No data" while loading
        return (
            <div className="chart-wrapper" id={chartInfo.chartID}>
                <div className="header">
                    <div className="right">
