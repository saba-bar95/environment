...
const LineChartKurtsik = ({ chartInfo }) => {
        ...
        const processedData = yearData
          .map(({ year }) => {
            ...
            selected.forEach((text) => {
              // NEW: Treat 0 or undefined/null as null to hide 0 points
              const val = dataItem[String(text.id)];
              dataPoint[text.name] = val === 0 || val == null ? null : val;
            });
            return dataPoint;
          })
          .filter(Boolean) // Existing: Remove null dataItems
          // NEW: Filter out years where ALL selected lines have null (i.e., all were 0/no data)
          .filter((dataPoint) => {
            return selected.some((text) => dataPoint[text.name] != null);
          });
          ...
          {selectedTexts.map((text, index) =>
            visibleLines[text.name] ? (
              <Line
                ...
                // OPTIONAL: Add this if you want lines to connect over null gaps (e.g., middle 0s)
                // connectNulls={true}
              />
            ) : null
          )}
...
export default LineChartKurtsik;
